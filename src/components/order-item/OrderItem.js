        ...
        {data && <Col>
            <Row className={clsx(style.orderItem, "align-items-center")}>
                ...
                <Col sm={8}>
                    ...
                    <button className={clsx(style.optionBtn)} onClick={() =>changeVariation()}>
                        <span>{data.productItem.options.map(option => option.value).join(" , ")}</span>
                        <i className="fi fi-rr-angle-small-down"></i>
                    </button>
                    <InputNumber onChange={updateQty} value={data.qty} className={clsx(style.quantityChange)} min={1}/>
                </Col>
                <Col sm={1} className={clsx(style.deleteBtn)} onClick={deleteItem}><i className="fi fi-br-cross-small"></i></Col>
            </Row>
